...
# Exemplos de uso:
#   gradiente2(nlp);
#   gradiente2(nlp, x0=[1;1]);
#   gradiente2(nlp, eps=1e-8);
#   x, f, gradnorm, iter, status, histf, histgradnorm = gradiente2(nlp, x0=[1;1], eps=1e-8, maxiter=2000);
# onde 'nlp' é a estrutura MathOptNLPModel do problema.
###################################################

# carrega pacotes necessários
using JuMP, NLPModels, NLPModelsJuMP, Printf, LinearAlgebra, Plots


# FUNÇÃO PRINCIPAL
function gradiente2(nlp; x0=nothing, eps=1.0e-6, maxiter=1000)
    ...
    # guarda o histórico de f e |gradf| para plotagem de graficos
    histf        = [f]
    histgradnorm = [gradnorm]
    ...
    return x, f, gradnorm, iter, status, histf, histgradnorm
end

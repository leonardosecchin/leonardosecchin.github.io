...
* **IMPORTANTE:** Somente instâncias definidas pela localização das cidades no plano são suportadas pelos códigos Octave/Matlab do curso (TSP Euclideano). Um exemplo é a instância `berlin52.tsp`. Neste caso, as linhas que não contém localização de cidades nos arquivos `.tsp` devem ser excluídas para funcionarem.
